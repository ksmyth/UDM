...
//<udmoclpat changes
	...
	PrintNode* TreeManager::CreatePrintNode() const
	{
		return new PrintNode( (TreeManager*) this );
	}

	FileNode* TreeManager::CreateFileNode() const
	{
		return new FileNode( (TreeManager*) this );
	}

	HandleNode* TreeManager::CreateHandleNode() const
	{
		return new HandleNode( (TreeManager*) this );
	}

	EnumerationNode* TreeManager::CreateEnumerationNode() const
	{
...
//##############################################################################################################################################
	...
	TextNode::TextNode( TreeManager* pManager )
		: TreeNode( pManager, TreeNode::NK_TEXTNODE ), m_strValue( "" ), m_pTreeNode( NULL )
	{
	}
	...
	bool TextNode::CheckImplementation( TypeContext& context )
	{
		...
		if ( ! m_pTreeNode ) {
			...
			m_pTreeNode = pObject;
			pObject->m_bTester = false;
			pObject->m_strType = "ocl::Boolean";
			pObject->m_strValue = "true";
			pObject->m_bCallable = false;
		}

		bool bResult = m_pTreeNode->Check( context );
		if ( bResult )
			m_vecType = m_pTreeNode->m_vecType;
		return bResult;
	};
...
//##############################################################################################################################################
	...
	EnumerationNode::EnumerationNode( TreeManager* pManager )
		: TreeNode( pManager, TreeNode::NK_ENUMERATIONNODE )
	{
	}
	...
	bool EnumerationNode::CheckImplementation( TypeContext& context )
	{
		bool bResult = true;
		int i;
		for ( i = 0 ; i < m_vecNodes.size() ; i++ ) {
			bResult = m_vecNodes[ i ]->Check( context )  && bResult;
		}
		if ( bResult )
			m_vecType = m_vecNodes[ i-1 ]->m_vecType;
		return bResult;
	}

	OclMeta::Object EnumerationNode::Evaluate( ObjectContext& context )
	{
		OclMeta::ObjectVector vecObjects;
		int i;
		for ( i = 0 ; i < m_vecNodes.size() ; i++ ) {
			vecObjects.push_back( m_vecNodes[ i ]->Evaluate( context ) );
		}
		return vecObjects[ i - 1];
	}
...
//##############################################################################################################################################
	...
	HandleNode::HandleNode( TreeManager* pManager )
		: TreeNode( pManager, TreeNode::NK_HANDLENODE ), m_pTreeNode( NULL )
	{
	}
	...
	bool HandleNode::CheckImplementation( TypeContext& context )
	{
		if ( ! m_pTreeNode ) {
			ObjectNode* pObject = GetTreeManager()->CreateObject();
			m_pTreeNode = pObject;
...
//##############################################################################################################################################
	...
	FileNode::FileNode( TreeManager* pManager )
		: TreeNode( pManager, TreeNode::NK_FILENODE ), m_pTreeNode( NULL )
	{
	}
	...
	bool FileNode::CheckImplementation( TypeContext& context )
	{
		bool bResult = m_pFileNameNode->Check( context );
		if ( ! m_pTreeNode ) {
			ObjectNode* pObject = GetTreeManager()->CreateObject();
			m_pTreeNode = pObject;
			pObject->m_bTester = false;
		...
		bResult = bResult && m_pTreeNode->Check( context );
		if ( bResult )
			m_vecType = m_pTreeNode->m_vecType;
		return bResult;
	}

	OclMeta::Object FileNode::Evaluate( ObjectContext& context )
	{
		OclMeta::Object spResult = m_pFileNameNode->Evaluate( context );
		if ( spResult.GetImplementation()->GetTypeName() == "ocl::String" ) {
			DECL_STRING( strValue, spResult );
		...
		if(strMode == "o") {
			...
			fo->open(strFileName.c_str(), std::ios::out | std::ios::app);
		} else {
			std::ifstream fi;
			fi.open(strFileName.c_str());
			if(fi) {
...
//##############################################################################################################################################
	...
	PrintNode::PrintNode( TreeManager* pManager )
		: TreeNode( pManager, TreeNode::NK_PRINTNODE ), m_pTreeNode( NULL )
	{
	}
	...
	bool PrintNode::CheckImplementation( TypeContext& context )
	{
		bool bResult = m_pNode->Check( context );
		if ( ! m_pTreeNode ) {
			ObjectNode* pObject = GetTreeManager()->CreateObject();
			m_pTreeNode = pObject;
			pObject->m_bTester = false;
	...
	OclMeta::Object PrintNode::Evaluate( ObjectContext& context )
	{
		OclMeta::ObjectVector vecObjects;
		OclMeta::Object spResult = m_pNode->Evaluate( context );

		if ( spResult.GetImplementation()->GetTypeName() == "ocl::String" ) {
			DECL_STRING( strValue, spResult );
			if(PatHelper::f_pat_output != NULL && *(PatHelper::f_pat_output) && PatHelper::f_pat_output->is_open()) {
				*(PatHelper::f_pat_output) << strValue.c_str();
			} else {
				fprintf( stdout, "%s", strValue.c_str() );
			}
		}
		else if ( spResult.GetImplementation()->GetTypeName() == "ocl::Boolean" ) {
			DECL_BOOLEAN( bValue, spResult );
			if(PatHelper::f_pat_output != NULL && *(PatHelper::f_pat_output) && PatHelper::f_pat_output->is_open()) {
				*(PatHelper::f_pat_output) << (bValue?"true":"false");
			} else {
				fprintf( stdout, "%s", (bValue?"true":"false") );
			}
		}
		else if ( spResult.GetImplementation()->GetTypeName() == "ocl::Real" ) {
			DECL_REAL( dValue, spResult );
			if(PatHelper::f_pat_output != NULL && *(PatHelper::f_pat_output) && PatHelper::f_pat_output->is_open()) {
				*(PatHelper::f_pat_output) << dValue;
			} else {
				fprintf( stdout, "%f", dValue );
			}
		}
		else if ( spResult.GetImplementation()->GetTypeName() == "ocl::Integer" ) {
			DECL_INTEGER( iValue, spResult );
			if(PatHelper::f_pat_output != NULL && *(PatHelper::f_pat_output) && PatHelper::f_pat_output->is_open()) {
			...
			if(PatHelper::f_pat_output != NULL && *(PatHelper::f_pat_output) && PatHelper::f_pat_output->is_open()) {
				*(PatHelper::f_pat_output) << "[undefined]";
			} else {

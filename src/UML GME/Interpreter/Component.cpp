...
{	
	try
	{
		...
		if(packages->GetCount() == 1)
		{
			...
			if(isUMT)
			{	
				CString sysCall = "call \"%UDM_PATH%\\bin.net\\Udm.exe\" \"" + filepath + "\"";
...
void CComponent::DumpCrossXML(std::strstream & xml, const CString & version)
{
	...
	xml << "<Uml:Diagram name =\"" << (LPCTSTR)name << "\"" << std::endl <<
	"version=\"" << (LPCTSTR)version << "\"" << std::endl << 
	"xmlns:Uml=\'http://www.isis.vanderbilt.edu/2004/schemas/Uml\' " << std::endl << 
	"xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"" << std::endl <<
	"xsi:schemaLocation=\'http://www.isis.vanderbilt.edu/2004/schemas/Uml Uml.xsd\'>" << std::endl << std::endl;
	...
	while (pos)
	{
		...
		namespaces.insert((const char*)ccdb->GetNamespace());
	};
	std::set<std::string>::iterator ns_i = namespaces.begin();
	xml << "<Uml:Namespace name=\"" << (LPCTSTR) name << "\">" << std::endl ;
	...
	xml << "</Uml:Namespace>" << std::endl;
	xml << "</Uml:Diagram>" << std::endl;
	//xml.close();
...
void CComponent::DumpIsolatedXML(std::strstream & xml, const CString & version)
{
	...
	while (ns_i != namespaces.end())
	{
		...
		CString ns = ns_i->c_str();
		xml << "<Uml:Namespace name=\"" << (LPCTSTR) ns << "\">" << std::endl ;
...
void CComponent::DumpCrossClasses(std::strstream &xml)
{
	...
	cont_class << "<Uml:Class _id=\"" << (LPCTSTR)cont_class_id << "\" name=\"_gen_cont\" isAbstract=\"false\"";  
	...
	while(pos)
	{
		...
		{
			compositions << "<Uml:Composition>" << std::endl;;
		
			compositions << "\t<Uml:CompositionChildRole _id=\"" << (LPCTSTR)childrole_id << "\" target=\"" << (LPCTSTR)comp_id << "\" min=\"0\" max=\"-1\"/>" << std::endl;
			compositions << "\t<Uml:CompositionParentRole _id=\"" << (LPCTSTR)parentrole_id << "\" target=\"" << (LPCTSTR)cont_class_id << "\"/>" << std::endl;
			compositions << "</Uml:Composition>" << std::endl;;	
...
*/
	...
	CString class_ph_name = GetName() + CString("_cross_ph_") + cls->GetClassDiagram()->GetNamespace();
	//--//CString class_ph_name = GetName();

	// if it's a derived class, no composition is generated - it uses the base's composition */
	if (has_base)
		xml << "<Uml:Class _id=\"" << (LPCTSTR) id << "\" name= \"" << (LPCTSTR)class_ph_name << "\" " << " isAbstract= \"false\" " ;
	else
		xml << "<Uml:Class _id=\"" << (LPCTSTR) id << "\" name= \"" << (LPCTSTR)class_ph_name << "\" " << " isAbstract= \"false\" childRoles=\"" << (LPCTSTR)role_id << "\"" ;
	...
	xml << " from=\"" << (LPCTSTR)(cls->GetPackage()->GetName())  << ":" << (LPCTSTR)(cls->GetClassDiagram()->GetNamespace()) <<"\" >" << std::endl;
	
	if (!has_base)
	{
		//the base class already has these attributes defined
		xml << "\t<Uml:Attribute name=\"rem_sysname\" type=\"String\" min=\"1\" max=\"1\" />" << std::endl;
		xml << "\t<Uml:Attribute name=\"rem_id\" type=\"Integer\" min=\"1\" max=\"1\"/>" << std::endl;
	}
	xml << "</Uml:Class>" << std::endl;
...
void CCompositeClass::DumpXML(std::strstream &xml, const CString& ns)
{
	...
	xml << "<Uml:Class _id=\"" << (LPCTSTR) id << "\" name= \"" << (LPCTSTR)GetName() << "\" ";
...
void CAssociationBase::DumpXML(std::strstream &xml, const CString& ns)
{
	...
	xml << "<Uml:Association _id=\"" << (LPCTSTR) id << "\"";
	if(!nm.IsEmpty())
		xml << " name= \"" << (LPCTSTR) nm << "\"";
	if(associationClass)
...
{
	out << "   <Uml:Constraint name=\"" << (LPCTSTR)GetName() << "\" description=\""
...
{
	out << "   <Uml:ConstraintDefinition name=\"" << (LPCTSTR)GetName() << "\" "
